...
def create_popularity_recommendation(train_data: pd.DataFrame, user_id: str, item_id: str, n: int = 10) -> pd.DataFrame:
    """
    Returns a popularity-based recommendation list based on number of plays of songs.
    
    Parameters:
    train_data (pd.DataFrame): pandas DataFrame containing data with columns user_id and item_id.
    user_id (str): column name in train_data that represents user IDs.
    item_id (str): column name in train_data that represents item IDs.
    n (int): number of recommendations to return.
    
    Returns:
    pd.DataFrame: a pandas DataFrame containing n item IDs with the highest recommendation scores based on popularity.
    """
